...
internal/
  ...
  server/             chi HTTP API + embedded web client
  store/              pgx pool + embedded goose migrations
web/                  static SPA (vanilla JS, embedded in the Go binary)
...
open http://localhost:8080   # log in with the demo credentials from `make seed`
